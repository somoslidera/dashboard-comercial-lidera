...
- **Modelo "resolve na leitura" + timing-proof (CONFIRMADO FUNCIONANDO):** conta NEGOCIAÇÕES (deal.id), não a pessoa. Webhook guarda só QUEM passou em cada etapa: `fxs:l:{mes}` `fxs:sql:{mes}` `fxs:r:{mes}` `fxs:d:{mes}` (SETs de **deal.id**) + `fxs:v:{mes}` (HASH deal.id→valor) + `dl:{deal.id}`=lead_id (mapa) + `banda:{deal.id}`=faixa. `porFaixaDoMes` agrupa pela `banda:{deal.id}` ATUAL; e para negociações ainda SEM faixa, **resolve ao vivo** na API (`faixaViaAPI` via `dl`→lead_id) e cacheia — isso resolve o TIMING (no `deal.created` a tag ainda não está na negociação; o Facebook a adiciona depois). Só calculado sob demanda (`?faixasMes=`).
- **Captura:** `marcarFaixaDeal(deal)` guarda `dl` e seta `banda` de TODAS as negociações do lead (recupera antigas quando o lead avança). `ressincronizarFaixa` (em `lead.tag_added`/`removed`) reconsulta e atualiza. Como agrupa na leitura, trocar a tag corrige tudo (contagens E faturamento).
- **RESSALVA:** negociações capturadas ANTES de existir o mapa `dl` ficam órfãs (não classificam até o lead ter atividade). Foi só um punhado de teste; toda negociação nova classifica sozinha.
