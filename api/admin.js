...
// Uso: /api/admin?token=SEGREDO&cmd=GET&key=v:count:2026-07
//      /api/admin?token=SEGREDO&cmd=INCRBY&key=v:count:2026-07&val=2
// Comandos: GET, SET, INCRBY, INCRBYFLOAT, DEL.  REMOVER depois de usar.
...
export default async function handler(req, res) {
  ...
  if (!OK.includes(cmd) || !q.key) {
    return res.status(400).json({ erro: 'use ?token=...&cmd=GET|SET|INCRBY|INCRBYFLOAT|DEL&key=...&val=...' });
  }
